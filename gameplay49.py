...
class CardPlotter(tk.Tk):
    def __init__(self, deck, button_text, attack_flag, deck_is_empty=False, no_more_cards_left=False, distance=2.5, factor=1):
        super().__init__()
...
if __name__ == "__main__":    
    ...
    while not done:
        counter +=1
        app = CardPlotter(deck, button_text="Finish", attack_flag= attack_flag, deck_is_empty=deck_is_empty, factor=3)
        app.mainloop()
        ...
        if deck_status == 0:
            deck_is_empty = True
            if not app.opponent_cards or app.my_cards:
                done = True
