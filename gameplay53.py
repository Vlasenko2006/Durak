...
class CardPlotter(tk.Tk):
    def __init__(self, deck, trump, button_text, attack_flag, deck_is_empty=False, no_more_cards_left=False, factor=1):
        super().__init__()
        ...
        self.frame = tk.Frame(self, bg='grey', width=frame_width, height=card_im.frame_height)
        ...
        self.minsize(frame_width, card_im.frame_height)
        ...
        self.create_closed_cards = card_im.create_closed_cards(self.opponent_cards,
                                                               self.image_refs,
                                                               self.upper_card_labels,
                                                               self.frame)
        # Create and place open card images at the bottom of the frame
        self.create_open_cards = card_im.create_open_cards(self.my_cards, 
                                                           self.image_refs,
                                                           self.lower_card_labels, 
                                                           self.on_card_click,
                                                           self.frame)
        # Draw the trump card perpendicular to the deck
        self.draw_trump_card_perpendicular = card_im.draw_trump_card_perpendicular(trump,
                                                                                   self.no_more_cards_left, 
                                                                                   self.image_refs,
                                                                                   self.table, 
                                                                                   self.frame)
    ...
    def on_card_click(self, event):
      ...
      #  print(f"Clicked label: {clicked_label}, Card info: {self.my_card}")
        ...
        relx_position = 0.1 + 0.05 * len(self.table_card_labels)
        clicked_label.place(in_=self.table, relx=relx_position, rely=0.5, anchor=tk.CENTER)
...
if __name__ == "__main__":    
    #  Defender gets more cards ! Even if deck is emty it gets all cards !!!!
    # insert the condition that cards in the deck distribute equally.
