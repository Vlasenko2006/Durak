...
class CardPlotter(tk.Tk):
    def __init__(self, button_text, attack_flag, deck_is_empty=False, no_more_cards_left=False, distance=2.5, factor=1):
        super().__init__()

        # Gameplay settings
        self.gamer = gamer_instance
        self.deck = self.gamer.deck
        ...
        card_im.create_closed_cards(self, self.num_closed_cards)
        # Create and place open card images at the bottom of the frame
        card_im.create_open_cards(self, self.my_cards, self.num_open_cards, self.row_spacing, self.on_card_click)
    ...
    def draw_trump_card_perpendicular(self):
        if not self.no_more_cards_left:
            trump_card_image = card_im.create_card_image("Q", "hearts")
    ...
    def finish_game(self, message="No message"):
        ...
        print(message)
        if self.done:
            self.my_cards.extend(self.cards_on_the_table)
        else:
            self.refill_hands()
        self.destroy()
