...
def game_turns(game, state_attacker, state_defender, deck, episode, game_log, attacker_net, defender_net):
    step_number = 0
    attack_value = None
    reward_attacker = 0
    reward_defender = 0
    ...
    while not done:
        ...
        # Attacker's turn
        valid_attacker_cards = [card for card in game.players[0] if card in deck and card not in attack_cards]
        if attack_value:
        ...
        if chosen_card not in game.players[0]:
            ...
            valid_defender_cards = [card for card in game.players[1] if (card[1] == chosen_card[1] and deck.index(card) > deck.index(chosen_card)) or (card[1] == game.trump and chosen_card[1] != game.trump) or (card[1] == game.trump and deck.index(card) > deck.index(chosen_card))]
            valid_defender_cards = [card for card in valid_defender_cards if card not in defense_cards]
            ...
            if chosen_defender_card not in game.players[1] or not game.can_beat(chosen_card, chosen_defender_card):
                ...
                defense_cards = []  # Reset defense cards
                done = True
