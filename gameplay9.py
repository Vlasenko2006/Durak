...
class CardPlotter(tk.Tk):
    def __init__(self, card_width, card_height, num_closed_cards, num_open_cards, distance=2.5):
        super().__init__()
        ...
        # Center the card rows
        self.center_card_row(num_closed_cards, row=0)
        self.center_card_row(num_open_cards, row=1)
    ...
    def center_card_row(self, num_cards, row):
        total_columns = 10  # Define a total number of columns to span
        pad_columns = (total_columns - num_cards) // 2
        for i in range(total_columns):
            self.frame.grid_columnconfigure(i, weight=1)
        for i in range(num_cards):
           # if self.frame.grid_slaves(row=row, column=i):
                self.frame.grid_slaves(row=row, column=i)[0].grid_configure(column=pad_columns + i)
    ...
    #         self.frame.grid_columnconfigure(i, weight=1)
    #     for i in range(num_cards):
    #         print("len = ", self.frame.grid_slaves(row=row, column=i)[0].grid())
    #         self.frame.grid_slaves(row=row, column=i)[0].grid(column=pad_columns + i)
    ...
    def on_card_click(self, event):
        ...
        middle_row = 1
        middle_column = 2
        pady_upwards_shift = int(self.card_height * 2)  # Adjust this value to shift more upwards
        ...
        # Wait for half a second (500 milliseconds) and then remove one black card from the top row
        self.after(500, self.remove_black_card_from_top)
        self.center_card_row(num_closed_cards, row=0)
        self.center_card_row(num_open_cards, row=1)

    def remove_black_card_from_top(self):
...
if __name__ == "__main__":
    ...
    num_closed_cards = 4  # User-specified number of closed cards
    num_open_cards = 7  # User-specified number of open cards
    app = CardPlotter(card_width, card_height, num_closed_cards, num_open_cards)
    app.mainloop()