...
def visualize_games(game_log):
    ...
    for i, log_entry in enumerate(game_log[:3]):
        ...
        if log_entry['attacker_action']:
            ...
            ax.text(0, 0.2, card_text, ha='center', va='center', fontsize=12, color='red')

        # Plot defending card
        if log_entry['defender_action']:
            card_text = f"{log_entry['defender_action'][0]} {suit_symbols[log_entry['defender_action'][1]]}"
            ax.text(0, -0.2, card_text, ha='center', va='center', fontsize=12, color='blue')
        ...
        ax.text(0, 0.35, remaining_attacker_text, ha='center', va='center', fontsize=10, color='black')
        ax.text(0, -0.35, remaining_defender_text, ha='center', va='center', fontsize=10, color='black')
