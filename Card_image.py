...
class Card_image:
    ...
    def create_closed_cards(self, card_plotter, num_closed_cards):
        for i in range(num_closed_cards):
            ...
            label = tk.Label(card_plotter.frame, image=closed_card_photo, bg='grey')
            label.image = closed_card_photo  # Store reference in label
            label.grid(row=0, column=i, padx=10, pady=10, sticky="n")
            card_plotter.image_refs.append(closed_card_photo)  # Store reference to prevent garbage collection
            card_plotter.upper_card_labels.append(label)  # Store reference to upper row card labels

        # Center the closed cards
        card_plotter.frame.grid_columnconfigure((0, num_closed_cards - 1), weight=1)
