...
def train_networks():
    ...
    for episode in range(num_episodes):
        ...
        # Log the trump suit at the start of the game
        game_log.append({
            'episode': episode + 1,
            'step': step_number,
            'trump': game.trump,
            ...
            'attacker_hand': list(game.players[0]),
            'defender_hand': list(game.players[1])
        })
        ...
        while not done:
            ...
            if chosen_card not in game.players[0]:
                ...
                continue_attack = torch.rand(1).item() > 0.5  # Randomly decide to continue or stop
            ...
            print("attacker chosen_card = ", chosen_card)
            game_log.append({
                'episode': episode + 1,
                'step': step_number,
                'trump': game.trump,
...
def visualize_games(game_data):
    ...
    for i, (attack_cards, defense_cards, trump_suit, step_number, reward_attacker, reward_defender, remaining_attacker_cards, remaining_defender_cards) in enumerate(game_data):
        ...
        remaining_attacker_text = "Remaining Attacker Cards: " + ", ".join([f"{card[0]} {suit_symbols[card[1]]}" for card in remaining_attacker_cards])
        remaining_defender_text = "Remaining Defender Cards: " + ", ".join([f"{card[0]} {suit_symbols[card[1]]}" for card in remaining_defender_cards])
        ...
        if reward_attacker == 100:
            ax.text(0.5, 1.5, "Attacker Wins", ha='center', va='center', fontsize=16, color='green')
        elif reward_defender == 100:
            ax.text(0.5, 1.5, "Defender Wins", ha='center', va='center', fontsize=16, color='blue')
